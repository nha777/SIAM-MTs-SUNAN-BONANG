...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-surface-900">Edit Role: {{ $role->name }}</h1>
    <p class="text-sm text-surface-500 mt-1">Perbarui peran dan izin aksesnya.</p>
</div>
        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-2">
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="block w-full rounded-md border-0 py-1.5 text-surface-900 shadow-sm ring-1 ring-inset ring-surface-300 placeholder:text-surface-400 focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm sm:leading-6 @error('name') ring-danger-300 focus:ring-danger-500 @enderror">
                    </div>
                    @error('name')
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative flex items-start">
                                    <div class="flex h-6 items-center">
                                        <input id="perm_{{ $permission->id }}" name="permissions[]" value="{{ $permission->name }}" type="checkbox" @checked(is_array(old('permissions', $rolePermissions)) && in_array($permission->name, old('permissions', $rolePermissions))) class="h-4 w-4 rounded border-surface-300 text-primary-600 focus:ring-primary-600">
                                    </div>
